...
export async function GET() {
  ...
  const company = getCompany(auth.user.companyId);
...
export async function PUT(request: Request) {
  const auth = await requireUser("agent");
  if (auth.error) return auth.error;
  const existing = getCompany(auth.user.companyId);
  ...
  return NextResponse.json(saveCompany({ ...existing, ...body, id: existing.id, name: body.name.trim() }));
}
